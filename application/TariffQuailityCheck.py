...
# Network tariffs
            ...
            with open(os.path.join('C:\\Users\\nhagh\\Codes\\Tariff-API\\application', 'AllTariffs_Network.json')) as NewTariffs:
                NewTariffs = json.load(NewTariffs)
            ...
            Timeind = pd.DataFrame(0, index=pd.date_range('2001-01-01', '2002-01-01', freq='30T'), columns=['ind1'])
            Timeind = Timeind.iloc[1:]
            Timeind['TS'] = pd.to_datetime(Timeind.index, unit='ms')
            # Timeind = Timeind.set_index('TS')
            FRR = 0
            for i in range(len(NewTariffs)):
                ...
                if tariff['Type'] == 'Flat_rate':
                    ...
                    for k1, v1 in tariff['Parameters'].items():
                        ti = 0
                        tariff2 = tariff['Parameters'][k1].copy()
                        for k, v in tariff2['Energy'].items():
                            this_part = tariff2['Energy'][k].copy()
                            ti += 1
                            for k2, v2, in this_part['TimeIntervals'].items():
                                start_hour = int(this_part['TimeIntervals'][k2][0][0:2])
                                if start_hour == 24:
                                ...
                                if this_part['Weekday']:
                                    if start_hour <= end_hour:
                                        Timeind[k1] = np.where((Timeind['TS'].dt.weekday < 5) &
                                                               (Timeind['TS'].dt.month.isin(this_part['Month'])) &
                                                               (((60 * Timeind['TS'].dt.hour + Timeind['TS'].dt.minute)
                                                                 >= (60 * start_hour + start_min)) &
                                                                ((60 * Timeind['TS'].dt.hour + Timeind['TS'].dt.minute)
                                                                 < (60 * end_hour + end_min))), Timeind[k1] + 1,
                                                               Timeind[k1])
                                    else:
                                        Timeind[k1] = np.where((Timeind['TS'].dt.weekday < 5) &
                                                               (Timeind['TS'].dt.month.isin(this_part['Month'])) &
                        ...
                        if len(Timeind[k1][Timeind[k1]< 1]) != 0:
                            print(tariff['Tariff ID'], ': Some times not assigned')
                            print(i)
                        elif len(Timeind[k1][Timeind[k1]> 1]) != 0:
                            print(tariff['Tariff ID'], ': Some times double assigned')
                            print(i)
                        else:
                            print(tariff['Tariff ID'], ': is all good!')